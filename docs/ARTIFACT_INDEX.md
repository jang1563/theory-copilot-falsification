...
| **`make` targets** | `Makefile` | `make venv`, `make smoke` (~1 min fast judge-visible confidence check on this laptop), `make test` (118/118 local-runnable as of 2026-04-25 package review), `make audit`, `make h1`, `make h2`, `make paper`, `make prereg-audit`, `make rejection-log`, `make skeptic-review` |
