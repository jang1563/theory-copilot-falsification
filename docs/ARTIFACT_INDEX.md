...
| **PhL-5** | TCGA-BRCA cross-cancer negative control (TOP2A−EPAS1 ccRCC-specificity check) | `preregistrations/20260423T224229Z_phl5_*.yaml`, `src/phl5_phl6_generalization_probes.py`, `results/track_a_task_landscape/external_replay/brca_cross_cancer/SUMMARY.md` | **pre-reg FAIL** (Δbase +0.009, ccRCC-specific reinforced) |
| **PhL-6** | GSE53757 third independent cohort (microarray platform shift) | `preregistrations/20260423T224229Z_phl6_*.yaml`, same probe script, `results/track_a_task_landscape/external_replay/gse53757/SUMMARY.md` | T-vs-N: **informative FAIL** (platform saturation, CA9 0.995); stage 1-2 vs 3-4: **PASS** AUROC 0.714 |
| **CPTAC-3** | Proteogenomic cross-platform replay — 6th verdict in chain (2026-04-26) | `results/track_a_task_landscape/external_replay/cptac3/`, `results/track_a_task_landscape/external_replay/cptac3_gate_result.json` | **honest FAIL** (ci_lower=0.542 < 0.60; Δbase=−0.007; direction p=0.006 preserved — gate refuses cross-platform replication, correctly) |
...
| **PhL-8** | Claude Code Routines `/fire` LIVE — Path C proof-of-life with recorded session ID (live view requires Claude account login; full event log retained as static artifact) | `src/phl8_routine_fire_live.py`, `routines_client.py`, `results/live_evidence/phl8_routine_fire/SUMMARY.md` | 200 OK + recorded `session_*` ID |
...
| **`make` targets** | `Makefile` | `make venv`, `make smoke` (~1 min fast judge-visible confidence check on this laptop), `make test` (107 local-runnable tests in the current target), `make audit`, `make h1`, `make h2`, `make paper`, `make prereg-audit`, `make rejection-log`, `make skeptic-review` |
| **Data provenance** | `data/SHA256SUMS` + `docs/public_data_provenance.md` | 17 CSVs hashed; every CSV's upstream source / access tier / builder script / submission role listed in the provenance doc; reviewer can `shasum -c data/SHA256SUMS` offline |
