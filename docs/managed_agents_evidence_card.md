...
| **Path C — API trigger (PhL-8)** | Claude Code Routine `/fire` live HTTP 200 + recorded `claude_code_session_url` (login required for live view; full event log in static artifact) | `session_01NyS541H3qZfJgqFVgWDcoM` | [`results/live_evidence/phl8_routine_fire/SUMMARY.md`](../results/live_evidence/phl8_routine_fire/SUMMARY.md) |
...
| **PhL-8** | 2026-04-23 ~23:10 | 1 Routine `/fire` (API trigger) | `session_01NyS541…` | HTTP 200 + recorded session ID `session_01NyS541H3qZfJgqFVgWDcoM` (login required; static event log retained) | fire + fork, no wait | 200 OK |
...
| **PhL-10 (oracle)** | 2026-04-26 (stage oracle) | 1 Routine API trigger (stage oracle, new Routine) | `session_01XGse8XYFtv3C1aKLZeMH9t` | 2 equations: CCNB1/PGK1 (FAIL, delta_baseline=0.007) + CXCR4/EPAS1 (PASS, AUROC 0.696, ci_lower=0.649, Δbase=+0.051); n=512 Stage I-II vs III-IV. Note: CLAIM_LOCK locks "KIRC Stage top AUROC = 0.689" (from stage_expanded local PySR run); 0.696 is the Routine oracle run result on the same equation — two different executions of the same law. | ~6 min | **FAIL + PASS** — second live oracle session |
