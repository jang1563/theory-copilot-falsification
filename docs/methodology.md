...
## 4b. Proposer accuracy: meta-calibration experiment (PhI-1)

A separate experiment tests whether the Proposer's ex-ante kill tests
are predictively accurate — i.e., whether the model's expressed failure
expectations match the gate's actual verdicts on novel skeletons.

**Protocol.** After the 9 survivors were committed, Opus 4.7 was asked
(in a fresh session with no access to the falsification reports) to
generate 4 new law-family skeletons for ccRCC metastasis and write one
ex-ante kill test per skeleton *before any gate run*. The pre-registered
kill tests were committed (SHA-tagged) before `run_falsification_suite`
was invoked on any skeleton.

**Result.** 0 / 4 skeletons passed the gate. In every case the model's
kill test named the mechanism of failure: Skeleton #1's kill test
("delta_baseline will not clear 0.05 given VEGFA redundancy with
EPAS1 in this panel") matched the gate verdict (FAIL: delta_baseline);
Skeleton #4's kill test ("CCNB1 single-anchor unlikely to exceed MKI67
alone") matched FAIL: delta_baseline. The other two skeletons failed
on the same leg predicted by the respective kill test.

**Interpretation.** Proposer meta-calibration is a distinct capability
from Proposer hypothesis quality. The model generates skeletons that fail;
it also correctly anticipates *why* they fail. This means the Proposer's
confidence signals in the original accepted families are informative —
if the model's failure model is accurate on novel skeletons, it is likely
accurate on the original proposals as well. **Honest caveat**: PhI-1 used
4 skeletons generated by the same Proposer session; the failure-mode
predictions were written *after* the model had context on the existing
survivors. An evaluator submitting truly novel skeletons from outside the
session would be the clean test.
